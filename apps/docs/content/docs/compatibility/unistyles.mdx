...
module.exports = {
  plugins: [
    ['react-native-boost/plugin', { integrations: { unistyles: 'on' } }],
    ['react-native-unistyles/plugin', { root: 'src' }],
  ],
};
```

Set `integrations.unistyles` to `off` when Unistyles is installed in your project's dependencies, but not actually used in code. The order of the two plugins does not matter.
