...
React Native Boost is compatible with Unistyles v3. Previous versions of Unistyles are untested and not officially supported.

## Setup

Enable Boost's Unistyles support layer in `metro.config.js`:
...
Set `integrations.unistyles` to `off` when Unistyles is installed in your project's dependencies, but not actually used in code.
