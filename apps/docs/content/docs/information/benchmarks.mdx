...
**baseline** (stock React Native), a **core-optimized** configuration that turns on React Native's own
...
In our benchmark, the **core-optimized** series in the graphs enables `reduceDefaultPropsInText`. Under heavy render load it recovers roughly **8–15%** of the baseline's lost frame rate on iOS (a bit more on Android), but closes only part of the gap. `NativeText`, the component React Native Boost uses under the hood, stays well ahead — around a **50%** margin over the core-optimized build at the heaviest loads on iOS. As React Native lands more of these optimizations, we expect the core-optimized line to keep rising toward React Native Boost.
