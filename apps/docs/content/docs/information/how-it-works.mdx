...
React Native Boost analyzes your code at build time and replaces these components with their native equivalents fully automatically. The plugin performs static analysis on your code to determine when it's provably safe to optimize a component and when it's not.

Optimized components are imported from `react-native-boost/runtime`, not directly from `react-native`, which allows us graceful fallback for web targets
...
If any safety requirement is not met, the component is left unchanged, preventing behavioral changes and other bugs.
