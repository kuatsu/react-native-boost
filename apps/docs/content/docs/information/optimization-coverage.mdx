...
| --- | --- | --- |
| `Text` | Imported from `react-native`, no blacklisted props, primitive children, safe ancestor chain | `contains blacklisted props`, `has Text ancestor`, `has unresolved ancestor that may render Text`, `has unresolved runtime parent that may render Text`, `contains non-primitive children`, `is a direct child of expo-router Link with asChild` |
...
Set `assumptions.unknownAncestorsDoNotRenderText` to `true` to optimize components under unknown ancestors or runtime parents across the project. This can produce the wrong native host if one provides a `Text` context.
...
The project-wide unknown ancestor assumption can optimize these cases. A false assumption can introduce regressions. For example, `expo-router`'s `Link` wraps its children in a `Text` by default. A `Text` under that `Link` must render as `NativeVirtualText`.
