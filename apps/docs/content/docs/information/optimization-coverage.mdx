...
| `Image` | Opted in with `optimizations.image: true`, imported from `react-native`, native platform known, supported source/style props, safe ancestor chain | `target platform is unknown`, `Image optimization is not supported in Unistyles mode`, `contains unsupported Image props`, `has a spread that may carry Image wrapper props`, `has Text ancestor`, `has unresolved ancestor and dangerous optimization is disabled` |
...
`Image` optimization is opt-in for now because it uses React Native's internal Image host path, which may print
deep-import deprecation warnings in newer React Native versions.
...
source/style/accessibility props can be reproduced safely. It skips Image in Unistyles mode because Unistyles does not
currently expose a lean Image host for shadow-tree registration.
