...
| `ActivityIndicator` | Imported from `react-native`, native platform known, direct props only, no children, safe ancestor chain | `target platform is unknown`, `has spread props`, `contains children`, `contains an impure prop expression that cannot be safely reordered`, `has Text ancestor`, `has unresolved ancestor and dangerous optimization is disabled` |
...
<Callout type="warn" title="Dangerous Mode">
  ...
  wrapper. For example, `expo-router`'s `Link` wraps its children in a `<Text>` by default. A `Text` under a `Link`
...
Set `dangerouslyOptimizeActivityIndicatorWithUnknownAncestors: true` to optimize unknown ancestors too. This has the same Text-ancestor risk as the View option.
