...
description: A technical tour of React Native wrapper overhead and how React Native Boost removes it at build time.
---
...
`NativeImage`. Static values are processed at build time. This includes parsing static `srcSet` density candidates with the detected React Native version and
hoisting the generated source list. Dynamic values use small runtime helpers. Compared to `Text` and `View`, the host component for `Image` is not imported directly. React Native does not export it through a supported interface, and only exports it through deprecated deep imports which log warnings when used. The runtime loads the public `Image` module to register `RCTImageView`, then renders that registered host directly.

### ActivityIndicator

`ActivityIndicator` normally renders an outer `View`, an iOS spinner host or Android `ProgressBarAndroid` wrapper, and then the platform host. Boost emits the outer native `View` and platform host directly. It inlines static defaults, sizes, and styles. Dynamic direct props use small helpers. Similar to `Image`, `ActivityIndicator` is registered manually without a deep import.
...
The Metro integration injects one Babel visitor on `JSXOpeningElement`. For each element it runs the `Text`, `View`, `Image`, and `ActivityIndicator` optimizers.

<Mermaid
  chart={`flowchart TD
  el["JSXOpeningElement (supported component)"] --> g{"Resolves to the react-native component?"}
...
The most intricate check is shared by the optimizers. Components under a `Text` can need different
...
The `unknown` case is often safe in practice because third-party components rarely wrap children in `Text`, but there are still cases where this can cause regressions. The project-wide `assumptions.unknownAncestorsDoNotRenderText` option provides an opt-in escape hatch to enable optimizations for this case. See [Configuration](/docs/configuration/configure).
...
- **`ActivityIndicator` runtime** registers the platform host through the public module. Small helpers preserve dynamic default, size, and style behavior. Static cases do not call them per render.
...
[`@boost-force`](/docs/configuration/boost-decorator#boost-force) on one element, or the
`assumptions.unknownAncestorsDoNotRenderText` option for the full project. Both make you responsible for the resulting behavior.
...
<Cards>
  ...
  <Card
    ...
    description="Plugin options, integrations, and project-wide assumptions."
  />
</Cards>
