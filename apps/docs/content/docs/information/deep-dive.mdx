...
Compared to `Text` and `View`, the host component for `Image` is not imported directly. React Native does not export it through a supported interface, and only exports it through deprecated deep imports which log warnings when used. The runtime loads the public `Image` module to register `RCTImageView`, then renders that registered host directly.
...
`View`, `Image`, and `ActivityIndicator` optimizers; each follows the same shape.
