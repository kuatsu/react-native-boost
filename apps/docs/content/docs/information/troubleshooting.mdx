...
A root component can be mounted inside a `Text`. Boost keeps a root `Text` wrapper so it can select `NativeText` or `NativeVirtualText`. It also keeps a root `View` with children so the wrapper can reset inherited text context.
