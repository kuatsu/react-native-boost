...
Several standard components in React Native such as `Text` and `View` are actually JavaScript-based wrappers around their native counterparts, `TextNativeComponent` and `ViewNativeComponent`. These wrappers are responsible for handling edge cases, such as Views nested inside Text components, pressable Text components, and more. These edge cases often only make up an incredibly small fraction of the usage of those components in your app, but require quite a lot of overhead to handle.

React Native Boost's Babel plugin statically analyzes your code to find usages of those components that can safely be replaced with the native components. If it finds any components that require the JS-based wrappers, it will keep them in place, ensuring that your app's behavior is not affected.

To give you an example, this is what React Native's `Text` component looks like (taken from [here](https://github.com/facebook/react-native/blob/main/packages/react-native/Libraries/Text/Text.js)). You can see that it has a lot of work before it renders the underlying native component – React Native Boost will often let you skip all of that:
