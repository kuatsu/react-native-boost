...
const pluginOptionKeys = ['optimizations', 'assumptions', 'integrations', 'ignores', 'logLevel'];
const optimizationKeys = ['native-text', 'native-view', 'native-image', 'native-activity-indicator'];
...
export function validatePluginOptions(rawOptions: unknown): PluginOptions {
  ...
  validateKeys(options, pluginOptionKeys, 'plugin option', legacyPluginOptions);
...
function validateKeys(
  ...
  for (const key of Object.keys(value)) {
    ...
    if (replacement) {
      throw new PluginError(`The \`${key}\` ${label} was removed. ${replacement}`);
    }
