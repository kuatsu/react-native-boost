...
const optimizeNativeView: JSXOptimizer = (path, { logger, options, unistylesEnabled, platform }) => {
  ...
  const hasVariableTextContext = () => getAncestor() === 'text' || getAncestor() === 'context';
  const needsRuntimeTextContext = () =>
    hasChildren(path) &&
    (hasVariableTextContext() ||
      (options?.assumptions?.unknownAncestorsDoNotRenderText !== true && getAncestor() === 'runtime'));
  ...
  const overridableChecks: BailoutCheck[] = [
    ...
    },
    {
      ...
      shouldBail: () => getAncestor() === 'unknown' && options?.assumptions?.unknownAncestorsDoNotRenderText !== true,
    },
  ];
