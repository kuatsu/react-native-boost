...
const computed = Platform.select({
  [key]: 'computed',
  ios: 'ios',
});
const spread = Platform.select({
  ...spec,
  ios: 'ios',
});
...
const prototype = Platform.select({
  __proto__: null,
  ios: 'ios',
});
const extraArgument = Platform.select(
  {
    ios: 'ios',
  },
  sideEffect()
);
const loose = 'ios' == 'ios' ? 'yes' : 'no';
const dynamic = 'ios' === target ? 'yes' : 'no';
const version = Platform.Version === 'ios' ? 'yes' : 'no';
const constants = Platform.constants === 'ios' ? 'yes' : 'no';
const otherSelected = OtherPlatform.select({
  ios: 'ios',
});
